  ...
  const drawChart = (angle) => {
    if (chartRef.current) {
      ...
      gradDef
        .append("stop")
        .attr("offset", "0%")
        .attr("style", "stop-color:rgb(107, 183, 248);stop-opacity:1");

        gradDef
        .append("stop")
        .attr("offset", "30%")
        .attr("style", "stop-color:rgb(173, 118, 199);stop-opacity:0.8");
      gradDef
        .append("stop")
        .attr("offset", "70%")
        .attr("style", "stop-color:#eb4ba2;stop-opacity:0.8");
      gradDef
        .append("stop")
        .attr("offset", "100%")
        .attr("style", "stop-color:#fc0d67;stop-opacity:1");
      const arc = d3
        .arc()
        .innerRadius(100)
        .outerRadius(150)
      ...
      svg
        .append("text")
        .text(
          `${((((angle - 220 + 360) % 360) / 280) * 40 + 30).toFixed(0)} °C`
        )
